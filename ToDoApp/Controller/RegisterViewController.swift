...
extension RegisterViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        let image = info[.originalImage] as! UIImage //Burada image dizisinden gelen orijinal fotoğrafı alıyoruz ve bu fotoğrafı UIImage türünden alıyoruz.
        self.profileImage = image
        cameraButton.setImage(image.withRenderingMode(.alwaysOriginal), for: .normal) //Butona bu fotoğrafı set ediyoruz. Yani artık butonun üzerinde bu fotoğraf konuluyor.
        cameraButton.clipsToBounds = true //Tüm bükülmeleri bükerek frame'e göre ayarlama
        cameraButton.layer.cornerRadius = 150/2 //Dairesel olması için yaptık
        cameraButton.contentMode = .scaleAspectFill
        cameraButton.layer.borderColor = UIColor.white.cgColor
        cameraButton.layer.borderWidth = 4
        self.dismiss(animated: true) //Fotoğrafı seçtikten sonra galeri sheet otomatik kaybolsun.
    }
}
